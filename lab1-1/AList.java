...
class AList<T> implements ListInterface<T> {
    ...
    public static void reverse(AList nums){
		for (int i = 0; i < nums.getLength()/2; i++) {
			nums.replace(i, nums.getEntry(nums.getLength()-1-i));
			nums.replace(nums.getLength()-1-i, nums.getEntry(i));
		}
	}
    public static void cycle(AList nums){

        
        for (int i = 1; i < nums.getLength(); i++) {
			nums.replace(i-1, nums.getEntry(i));
		}
        nums.replace(0,nums.getEntry(nums.getLength()+1));
