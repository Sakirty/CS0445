...
public class StackSort {
    ...
    //StackInterface
    //Stack
    VectorStack<Integer> dui = new VectorStack<Integer>();
    ...
    while(!dui.isEmpty()){
        ...
        if(temp2<temp){
            if(lowerValues.isEmpty()){
                lowerValues.push(temp2);
            }   else    {
                int temp2s = lowerValues.peek();
                if (temp2>temp2s){
                    lowerValues.push(temp2);
                }   else    {
                    int exchange = temp2s;
                    lowerValues.pop();
                    lowerValues.push(temp2);
                    lowerValues.push(temp2s);
                }
            }
        }
        else{
            if(upperValues.isEmpty()){
                upperValues.push(temp2);
            }   else    {
                int temp2s = upperValues.peek();
                if (temp2>temp2s){
                    upperValues.push(temp2);
                }   else    {
                    int exchange = temp2s;
                    upperValues.pop();
                    upperValues.push(temp2);
                    upperValues.push(temp2s);
                }
            }
        }
    }
    /*doStackSort(upperValues);
    doStackSort(lowerValues);
