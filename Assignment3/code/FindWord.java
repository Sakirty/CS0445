...
{	
	...
	public FindWord()
	{
		...
		while (!(word.equals("")))
		{
			...
			for (int r = 0; (r < rows && !found); r++)//this row not found, pass through all rows
			{
				for (int c = 0; (c < cols && !found); c++)//pass through all cols in one row
				{
			...
			if (found)//found and print out the location & word
			{
	...
	public boolean findWord(int r, int c, String word, int loc, char [][] bo)
	{
		...
		if (r >= bo.length || r < 0 || c >= bo[0].length || c < 0)
			...
			return false;//check boundary first
