...
// or null otherwise.
    ...
    public boolean equals(LinkedBag<T> aBag) {
        ...
        if (this.getCurrentSize() != aBag.getCurrentSize()){
            ...
            for( int i = 1; i < aBag.getCurrentSize(); i++);{
                if(this.getFrequencyOf(aNode.data) != aBag.getFrequencyOf(aNode.data))
                    return result = false;
            }
        }
    ...
    public void removeDuplicates() {
        ...
        // COMPLETE THIS METHOD 
         /* Check it's frequency
            while frequency > 1: remove the node
            Go to the next node. */
         Node scout = firstNode;
	        
         while(scout != null){
		      while(getFrequencyOf(scout.data) > 1){
			      remove(scout.data);
		      }	
                scout  = scout.next;
	      }
