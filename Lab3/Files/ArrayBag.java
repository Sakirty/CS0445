...
// equals bag[where]; otherwise, anEntry is not in the array
    ...
    public boolean equals(ArrayBag<T> aBag) {
       ...
       boolean result = true;//set the reault as true
       if(this.getCurrentSize() == aBag.getCurrentSize()){// if they are the same size compare them
         for( int i = 0 ; i < getCurrentSize(); i++){//go through
            if(this.getFrequencyOf(bag[i]) != aBag.getFrequencyOf(bag[i])){//if the frequency not same, return false
               result = false;
             }
            }
          }
          else{
            result = false;//if they dont have the same size return false
           }
         return result; 
    }  // end equals
    ...
    public boolean duplicateAll() {
        checkInitialization();
        boolean success = true; //set initial value success 
        
        int currSize = this.getCurrentSize();//get the size
        if(currSize *2 <= bag.length){
         for(int i = 0; i< currSize; i++){
               add(bag[i]);//duplicate all, the result of success is still true
          }
        }    
        else{
            success = false;//if fail return false
        }           
        return success;
    }  // end duplicateAll
    ...
    public void removeDuplicates() {
        ...
        for(int i = 0; i<this.getCurrentSize(); i++){
         while(this.getFrequencyOf(bag[i])>1){//if the frequence is larger than one
            remove(bag[i]);//remove it
          }
        }    
        return;
